...
export default function Home() {
  ...
  const edgeClip = !panelOpen && showWeb && (
    <button
      ...
      className={`flex h-20 w-5 shrink-0 items-center justify-center self-center border border-white/[0.06] bg-zinc-950/90 text-white/30 backdrop-blur-sm transition-all hover:w-6 hover:bg-zinc-900 hover:text-white/70 ${panelSide === "right" ? "rounded-l-lg border-r-0" : "rounded-r-lg border-l-0"}`}
      title="Open panel (⌘/)"
      aria-label="Open panel"
    >
      {panelSide === "right" ? <ChevronLeft className="h-4 w-4" strokeWidth={2} /> : <ChevronRight className="h-4 w-4" strokeWidth={2} />}
    </button>
  );
  ...
  const panelInner = showWeb && (
    <div className={`h-full overflow-hidden transition-[width] duration-300 ease-out motion-reduce:transition-none ${panelOpen ? panelWidth : "w-0"}`}>
      <div className={`h-full ${panelWidth} overflow-hidden border-white/[0.06] bg-zinc-950 shadow-2xl transition-transform duration-300 ease-out motion-reduce:transition-none ${panelSide === "right" ? "border-l" : "border-r"} ${panelOpen ? "translate-x-0" : panelSide === "right" ? "translate-x-full" : "-translate-x-full"}`}>
        <DeckPanel
  ...
  return (
    <div className={`flex h-screen w-screen overflow-hidden transition-colors duration-300 ${isDark ? "bg-black text-white" : "bg-white text-black"}`}>
      ...
      <div className="relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        {processing && (
          <div className={`absolute top-4 left-1/2 z-40 -translate-x-1/2 rounded-full border px-4 py-2 text-xs font-medium shadow-lg ${isDark ? "border-amber-400/40 bg-amber-500/15 text-amber-100" : "border-amber-600/30 bg-amber-50 text-amber-900"}`}>
            Updating…
          </div>
        )}
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 flex justify-end">
          <div className="pointer-events-auto mb-5 mr-8 flex items-center gap-3">
            <Link href="/settings" className="rounded-md border border-current/15 px-2.5 py-1 text-xs opacity-35 transition-opacity hover:opacity-75" title="Settings (⌘,)">
              Settings
            </Link>
            <button type="button" onClick={() => void exportPptx()} className="rounded-md border border-current/15 px-2.5 py-1 text-xs opacity-35 transition-opacity hover:opacity-75" title="Export (⌘E)">
              Export
            </button>
            <span className="text-sm tabular-nums opacity-25">{index + 1}/{slides.length}</span>
          </div>
        </div>
      </div>
